...
targetName = '20161031_2-4-6_data_Fr0.xlsx';
...
%         if not(isnan(colN(rCN,9)))
%             alpha(rr,s)=tempData(:,colN(rCN,9));
%         end
        if isnan(colN(rCN,4))
            ...
            for iq = rr
                ...
                Lw = 2*0.03; % experimentally found drawadown length (until contraction end)
...
for cType = 1:3
    ...
    xlswrite(targetName, [expNo(dataStartRow:dataStartRow+readLen-1,:), ...
                          ...
                          Fr(dataStartRow:dataStartRow+readLen-1,:), ...
...
%                           theta(dataStartRow:dataStartRow+readLen-1,:)],...
%                           1, ['B',num2str(startRow),':Y',num2str(readLen+startRow-1)]);
    startRow = startRow+readLen+5;
    dataStartRow = dataStartRow+readLen;
end
cd ..
cd('Plots\Unsteady') % store Fr and theta (steady for comparison plot)
Fr_steady = Fr;
theta_steady = theta;
h0_steady = h4;
hcr_steady = hcr;
Q_steady = Q;
save('SteadyData.mat','Fr_steady','theta_steady','hcr_steady','h0_steady','Q_steady')
cd ..\..
cd('Data')
