...
c_m = a_c*exp(b_c*Q)+c_c*exp(d_c*Q);
Jf = Jfm*ones(size(Jf));
dh(4) = -dx*Jfm;
...
for i = 1:4
    ...
    if not(startUpwind)
        ...
        egl(jStart) = wLvl(jStart)+(Q/A_ini)^2*(dx/c_m^2/Rh_ini+1/(2*g));
    end
    if not(i==1)
    ...
    c = a_c*exp(b_c*Q)+c_c*exp(d_c*Q)*ones(size(Jf));
    Jf = Jfm*ones(size(Jf));
...
for j =jEndC:NX(4)-1
    ...
    if not(stopTraj)
        ...
        Rhij = Aij/Pij;
        dEr = dx*Q^2*(1/(Aij^2*c(j)^2*Rhij));%-1/(Aij^2*c(j)^2*Rhij));
        %dEu = Q^2/(2*g)*(1/A0^2-1/Aij^2);
...
%         Aij = hij(posEndTraj)*(wij+hij(posEndTraj)/tand(alphaij));
%         Pij = wij+2*hij(posEndTraj)/sind(alphaij);
        Aij = hij(ijPos+1)*(wij+hij(ijPos+1)/tand(alphaij));
        Pij = wij+2*hij(ijPos+1)/sind(alphaij);
        Rhij = Aij/Pij;
